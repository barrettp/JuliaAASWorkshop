...
First install the packages: ModelingToolkit, Makie, and DifferentialEquations.
"""
...
md"""
A simple abstract component with two pins is called a 'TwoPin`. The equations specify that: (1) the voltage difference between the positive pin and negative pin is the voltage difference of the component, (2) the current between the two pins must sum to zero, and (3) the current of the component is that of the positive pin.

Note that constants and parameters use the `=` characater, while equations use the `~` character, which means equivalent.

!!! info

    Julia provides two types of inheritance: *type* inheritance and *method* inheritance. `TwoPin` is an example of method inheritance.
"""

# ╔═╡ 53fb1346-06ed-4258-ad41-79597f544dad
md"""

    @component function TwoPin(; name)
        @named p = Pin()
        @named n = Pin()
...
md"""
The first actual component is the Source, which generates a current and has a constant voltage. It can be thought of as a two pin component where the voltage is kept constant.

Note that we have provided the component with a default parameter for the source's voltage. It will be use if we don't provide a value. Also, note the use of `@unpack` and `extend`. \@unpack` means that we only want to use `TwoPin`'s equations and states and `extend` means that we want to extend the system of equations. 
    ...
    @component function Source(; name, V = 1.0)
        ...
        extend(ODESystem(eqs, t, [], ps; name = name), twopin)
    end
"""
...
md"""
The second component is the Resistor. It has two pins and the voltage obeys Ohm's law: `V = I*R`.
...
md"""
Now that all components are instantiated, we can connect them together to form a sinple RC circuit. Connect the positive pin of the source to the positive pin of the resistor, the negative pin of the resistor to the positive pin of the capacitor, etc.
...
u0 = [ capacitor.v => 0.0, capacitor.i => 0.0]
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "8a6e299ba21d2270478c077f4cd551a6b66f9252"
