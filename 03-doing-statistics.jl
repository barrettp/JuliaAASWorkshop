...
begin
	...
	using Distributions
	using LsqFit
	md""" #### The ten packages the notebook is `using` are called in this cell."""
end	
...
md"""
## 4 June 2023
Co-developed with Denis F. Cioffi, Ph.D.

###### This tutorial focuses first on reading and using a standard "Machine Readable Table" from _The Astrophysical Journal_.
  - [Here](https://journals.aas.org/mrt-overview/) is the link to the standard.
  - [Here](https://iopscience.iop.org/article/10.3847/1538-4365/acc1de#apjsacc1det1) is the link to download the table from the paper, "A Statistical Analysis of Galactic Radio Supernova Remnants," by S. Ranasinghe and D. Leahy, whom we acknowledge for communication that assisted our understanding.
###### Because a Julia Pluto notebook keeps track of all the packages, their dependencies, and their versions, it is completely reproducible: the notebook used by someone else is identical to the original notebook.
  - Here, an external file must be opened, so the path to that file is specific to any given system (but a checkbox allows for an easy switch between different paths on different computers.)

"""

# ╔═╡ b3031b04-70a0-4335-af56-685b75301765
	md"""
	### Preliminary Settings
	###### The following cell widens cell width proportionally to window width.
	"""

# ╔═╡ 43981055-f016-46e6-a019-9ac4501db0ad
begin # to widen cell width; https://discourse.julialang.org/t/cell-width-in-pluto-notebook/49761/4
	html"""
	<style>
		main {
			margin: 0 auto;
...
# ╔═╡ a7f8f920-5ee8-420a-bcb2-fcd30dde7d28
	md"""
	###### The `PlotThemes` package allows for selection of a dark background for plots.  If desired, check box:$(@bind darkTH CheckBox(default = false))
	"""
...
For the right path to the external file, we can set a `basePath` variable. If using a second computer with a different path, clearing the checkbox sets `DCServer` to false and selects the alternative path: $(@bind DCServer CheckBox(default=true))
"""

# ╔═╡ d68c548a-71a3-45e6-9ec5-a6d9358a716a
begin
	if DCServer
		basePath = "/home/dcioffi/Gitted/Illustris/Presentations/SNR_Statistics/"
	else
...
# ╔═╡ bd638878-52d4-4a33-adaa-052cf9c2a1c7
md"""
###### A notebook can have a table of contents (which can be collapsed) showing headings; clicking on a line brings one directly to that spot in the notebook.  The headings can be shown to a depth of six hashes (#) or fewer.  Here  it is set to three.
"""

# ╔═╡ 4d9eb5bd-0759-4499-bd42-621834ae7f67
TableOfContents(title = "SNR Statistics", depth = 3)

# ╔═╡ 0e4c1c56-f61e-4eed-a011-c08384d09dc0
md"""
## This MRTs module can read any ApJ machine-readable table.
  - ###### Note the use of "struct" to build variables containing different types ("Composite Types") and the use of "regexs" (Regular Expressions) and iterators to read the MRT file.
  - ###### It also takes advantage of the so-called ternary operator, `a ? b : c`, uses `nothing` and in particular, `missing`, which we address next.
  - ###### `eachline` sets up the iteration over the entire file.
"""
...
# ╔═╡ a05674c6-2c9f-4720-abc6-13539b0a0fb7
allInfo = MRTs.MRTable(basePath*"apjsacc1det1_mrt.txt")

# ╔═╡ 81265717-bcbf-4a70-a7a6-9f770f47ce42
begin
	nRows = length(allInfo.data)
	md"""
	#### We find $nRows rows of SNR data and clarifying notations.
	##### Note how the module that reads the MRT data takes advantage of Julia's "missing" singleton, which is a recommended Julia standard. Where data are not provided, one need not worry about any particular form or special symbol(s), and in what follows, we shall benefit from this uniformity.
	"""
end
...
goDownCol(col::Int64) = skipmissing([allInfo.data[i][col] for i = 1:nRows])
...
typeof(([allInfo.data[i][colRadius] for i = 1:nRows]))

# ╔═╡ cbaa2844-7054-4e8a-8d92-9706a6a49cef
typeof(skipmissing([allInfo.data[i][colRadius] for i = 1:nRows]))
...
# ╔═╡ cb719986-278d-4e90-b8a3-0de0d05467b0
ind_nonMissingRadii = findall(x -> !ismissing(x), goDownCol(colRadius))

# ╔═╡ b48e97b0-8e8e-4710-87fe-7e3a7bee007d
md"
### Collect the non-missing diameter indices; initially, we find $(length(ind_nonMissingRadii)).
...
md" ###### We need the index to the problematic SNR that is too close to the Galactic Center (G1.4 -0.1)."
...
md""" ###### Now we find the indices of the SNRs to be excluded because their distances are given only as limits, and then we use `symdiff` (if you will, the opposiste of `intersect`: "set of non-overlapping items between two sets") to remove them from the array of desired indices.
"""

# ╔═╡ bde10148-2a5f-4597-8e81-f54d71f05728
ind_UpperOrLowerLimits_OnDistance = findall(x -> x==">" || x=="<", goDownCol(colDistanceLimit))
...
md" ###### We go down the column again, but this time selecting only the indices that we have whittled down to the good ones, and we multiply those radii by 2 to get the SNR diameters. The yield is 215 SNRs."
...
##### Now change the y axis to a probability density function: `isdensity` becomes `true` when we "normalize" the fit: `h = normalize(yFit2, mode= :pdf)`.
  - And make the x axis more informative.
"""
...
plot(h, label = "", xlims = (firstEdge, lastEdge), xlabel = xAxisLabel_Dhist, ylabel = yAxisLabelPDF, xticks = (firstEdge:2*binSize:lastEdge), xminorticks = 2, widen = true)
...
begin
	...
	# plot!(yFit2.edges[1][1:18, 1], cubicScaled)
...
#### The "scaling" turns the interpolation into a function that can be evaluated at intermediate values to prodce a smooth line.
  - ###### Since the orignal has 18 points (18 bins, 19 edges), the scaling function requires 18; hence we start it at 1 (second edge divided by binSize: yFit2.edges[1][2])/binSize = $(yFit2.edges[1][2]/binSize)) and not at `firstEdge`.
"""
...
md"
- ###### As an example of how the new variable can be evaluated at any (internal) point, `cubicScaled(166)` = $(round(cubicScaled(166), digits = 5)).
...
begin
	...
	# plot!(0:176, cubicFiner, linewidth=2, color = :blue, label = "interpolated") 
	plot!(x -> cubicScaled(x), 0, 176, linewidth=2, color = :black, label = "interpolated")
	# plot!(x -> cubicScaled(x), 0, 176, linewidth=2, color = :white, label = "interpolated") # if dark background, want white line.
end

# ╔═╡ eeba065c-942e-46f0-a3eb-d89e90bc514a
md"
#### But suppose instead we want a smoothing (as in the figure in the paper) that does not hit the points exactly but instead, well, smooths them.
###### Again we need 18 points on the x axis, but now we want to start at zero, so we can use `yFit2.edges[1][1:18, 1]` or `firstEdge:binSize:lastEdge - 1`
"
...
# ╔═╡ ae34a53f-da42-42ac-b283-eefa09f3c86d
 md"""
!!! note "This smoothing function is based on:"
...
md" ###### And again we have the choice between plotting a new variable with a finer distribution or plotting the function directly, but with this package, the transition to a functional form of `smooth` is `predict(smooth, x)` "

# ╔═╡ 54fb1116-4887-4690-8c14-e70e206ce033
smoothFiner = [predict(smooth, i/1) for i = 0:176] # the `i/1` turns it into floating point
...
begin
	...
	# plot!(cubicFiner, linewidth = 2, label = "interpolated")
	plot!(x -> cubicScaled(x), 0, 176, linewidth=2, color = :black, label = "interpolated")
	# plot!(smoothFiner, linewidth = 2, color = :black, label = "smoothed")
...
## (Empirical) Cumulative Distribution Function
  - ###### With standard functional distributions, e.g., Normal, use the Distributions package.
    - ###### Here, the function `ecdf` could be used, resulting in a type "ECDF{Vector{Float64}, Weights{Float64, Float64, Vector{Float64}}}"
...
# ╔═╡ dd758a03-3f4b-42c1-8642-4c0809e8ff40
# not clear to me how this would be more helpful
let
# begin
	gcdf = ecdf(diameters)
	# gcdf.sorted_values
	# gcdf.weights
    # scatter(x -> gcdf(x), 0, 150)
	typeof(gcdf)
end
...
md"
###### As before, an interpolated version would allow determination at intermediate values, but scaling would not be needed because the interprolation is already scaled to the proper x-axis limits, i.e., `scatter(sortedD, interpECDF)` would reproduce the above plot.
"

# ╔═╡ 60363deb-8616-4942-8b7f-a9763ee2cc29
md"
## The SNR Σ -- D Relation 
"

# ╔═╡ a58f3563-ec5d-459d-89ea-892a964d53a6
md""" ###### All the SNRs with diameter estimations do not  have 1 GHz flux densities. A further subset of those that remain  are shell-type, and those are the ones we wish to plot.
...
ind_AgeLimits = findall(x -> x==">" || x=="<", goDownCol(colAgeLimit))

# ╔═╡ f2b86512-40a3-4f99-a0c3-937cfe684a14
begin
	ind_MissingAge = findall(x -> ismissing(x), [allInfo.data[i][colAge] for i = 1:nRows]) # can't use the function `goDownCol` becuase it uses `skipmissing`
...
md" ###### The extremes let us know immediately that we'll need to use the logarithms of the ages also."
...
md" ### We do a best fit on Σ--D and here plot the logarithms directly (instead of letting the plotting function take them). "

# ╔═╡ ade1768e-c5ee-4cd5-ad9f-19948265b9c3
begin
	XX = log10.(diametersToPlot)
	YY = log10.(ΣShellsToPlot)
...
begin
	@. model(t, p) = p[1] * t + p[2]
	p0 = [-1.5, 10]
	md" The straight-line model to be fit is here. "
end
...
begin
	...
	md" We use the results of the model to get the Σ values at the minimum and maximum diameters, which we then plot. "
end
...
## What Was Learned 
  - #### Prelims
    - ###### The Dates package can show the current date.
    - ###### html code used to widen cells proportionally to window width.
    - ###### The PlotThemes package allows for dark plots. 
    - ###### PlutoUI lets us make check boxes (for binary options) and have a Table of Conents.
  ...
  - #### Histogram and Cumulative Distribution Creation 
    ...
    - ###### The Interpolations package was used to create a function that reproduced the points.
...
# ╔═╡ d16f1391-34db-485d-a945-ce6774cffecb


# ╔═╡ d00f8fec-9ab4-40b1-9910-8ff3ee6775b8
md"""
### Last Cell
  - ###### I mark the "last cell" so I can jump to the end instantly.
"""
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "e877e42ea83f5a1ba96514842ec893b92bcee1c1"
